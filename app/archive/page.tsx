...
const categories=[{key:"all",label:"All"},{key:"content",label:"Content"},{key:"campaigns",label:"Campaigns"},{key:"products",label:"Products"},{key:"samples",label:"Samples"}] as const;
...
export default async function ArchivePage({searchParams}:{searchParams:Promise<{category?:string;q?:string}>}){
  ...
  const sections=[
    ...
    {key:"products",label:"Products",items:products.filter(x=>match(x.product.name)).map(x=>({id:x.product.id,name:x.product.name,meta:x.product.category||"Uncategorized",href:`/products/${x.product.id}`,action:restoreProductAction}))},
    {key:"samples",label:"Samples",items:samples.filter(x=>match(`${x.productName} ${x.sample.sourcePlatform||""}`)).map(x=>({id:x.sample.id,name:x.productName,meta:x.sample.sourcePlatform||x.sample.status,href:`/products/samples/${x.sample.id}`,action:restoreSampleAction}))}
