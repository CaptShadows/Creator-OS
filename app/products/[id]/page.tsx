...
import { addListingAction, archiveProductAction, permanentlyDeleteProductAction, updateProductAction } from "../actions";

export const dynamic = "force-dynamic";

export default async function ProductPage({ params }: { params: Promise<{ id: string }> }) {
  const owner = await requireOwner(), data = await getProductWorkspace(owner.id, (await params).id);
  if (!data || data.product.archivedAt) notFound();
