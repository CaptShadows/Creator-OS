...
export default async function CreatePage({ searchParams }: { searchParams: Promise<{ archived?: string;duplicateMatches?:string;duplicateToken?:string;idea?:string }> }) {
  ...
  return <section className="space-y-8">
    ...
    <div className="flex items-center justify-between"><h2 className="display-heading text-3xl">{showArchived ? "Archived content" : "Content workspace"}</h2><Link className="text-sm font-bold text-[var(--accent-strong)]" href={showArchived ? "/create" : "/create?archived=1"}>{showArchived ? "Back to active" : "View archive"}</Link></div>
    {items.length === 0 ? <EmptyState title={showArchived ? "Archive is empty" : "No ideas yet"} description={showArchived ? "Archived work will remain recoverable here." : "Use the one-field capture above. Nothing else is required."} /> : <div className="grid gap-3">{items.map((item) => <article key={item.id} className="luxury-card flex flex-wrap items-center justify-between gap-4 rounded-xl p-5"><div className="min-w-0"><div className="flex flex-wrap items-center gap-3"><h3 className="truncate font-bold">{item.title}</h3><StatusBadge>{contentStatusLabels[item.status]}</StatusBadge></div>{item.concept && <p className="mt-2 line-clamp-2 text-sm text-[var(--muted)]">{item.concept}</p>}</div>{showArchived ? <form action={recoverContentAction}><input type="hidden" name="contentId" value={item.id}/><button className="min-h-11 px-3 text-sm font-bold text-[var(--accent-strong)]">Recover</button></form> : <Link className="primary-action inline-flex min-h-11 items-center px-4 text-sm font-bold" href={`/create/${item.id}`}>Open</Link>}</article>)}</div>}
  </section>;
}
