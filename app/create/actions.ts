...
export async function createIdeaAction(formData: FormData): Promise<void> {
  const owner = await requireOwner(); const { idea } = quickIdeaSchema.parse({ idea: formData.get("idea") });const title=idea.split(/\r?\n/)[0].trim().slice(0,200)||"Untitled idea",matches=await findDuplicates(owner.id,"content",title);if(matches.length&&!validDuplicateToken(String(formData.get("duplicateToken")||""),owner.id,"content",title))redirect(duplicateWarningUrl("/create",{idea},duplicateToken(owner.id,"content",title),matches)); const id = await createQuickIdea(owner.id, idea); redirect(`/create/${id}`);
}
...
export async function permanentlyDeleteContentAction(formData:FormData){const owner=await requireOwner(),id=String(formData.get("contentId"));if(!await permanentlyDeleteContent(owner.id,id))redirect(`/create/${id}?deleteError=Remove%20publications%2C%20deliverable%2Fsample%20links%2C%20and%20PDFs%20before%20deleting.`);revalidatePath("/create");redirect("/create?deleted=1");}
