...
import { createAttachment, type AttachmentTarget } from "@/lib/attachments/repository";
...
export async function POST(request:Request){
  ...
  const file=form.get("file"),targetType=String(form.get("targetType")),targetId=String(form.get("targetId"));
  if(!(file instanceof File)||!targetTypes.has(targetType)||!targetId)return Response.json({error:"invalid_upload"},{status:400});
  try{const {config,storage}=configuredAttachmentStorage();if(file.size>config.maxBytes)throw new AttachmentValidationError(`The PDF exceeds the ${config.maxBytes}-byte upload limit.`);await createAttachment(owner.id,{type:targetType as AttachmentTarget["type"],id:targetId},{filename:file.name,mimeType:file.type,bytes:new Uint8Array(await file.arrayBuffer())},storage,config.maxBytes);return redirectBack(request,form.get("returnTo"),"uploaded");}
