...
export async function GET(_:Request,{params}:{params:Promise<{id:string}>}){
  ...
  try{const bytes=await configuredAttachmentStorage().storage.read(row.storageKey);return new Response(bytes,{headers:{"Content-Type":"application/pdf","Content-Length":String(bytes.byteLength),"Content-Disposition":`inline; filename*=UTF-8''${encodeURIComponent(row.originalFilename)}`,"Cache-Control":"private, no-store","X-Content-Type-Options":"nosniff"}});}catch{return Response.json({error:"backing_file_missing"},{status:410});}
}
