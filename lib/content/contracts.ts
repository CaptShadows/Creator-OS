...
export const contentAssociationsSchema = z.object({
  contentId: z.string().uuid(),
  campaignIds: z.array(z.string().uuid()),
  productIds: z.array(z.string().uuid()),
  platformAccountIds: z.array(z.string().uuid()),
});

export type ContentAutosaveInput = z.infer<typeof contentAutosaveSchema>;

export function pendingDraftKey(ownerId: string, contentId: string): string {
  return `creator-os:draft:${ownerId}:${contentId}`;
}
