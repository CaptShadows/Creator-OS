...
import { archiveAttachment, permanentlyDeleteAttachment, renameAttachment, restoreAttachment } from "./repository";
import{configuredAttachmentStorage}from"./storage";
...
export async function permanentlyDeleteAttachmentAction(f:FormData){const o=await requireOwner(),x=input.parse({attachmentId:f.get("attachmentId"),returnTo:f.get("returnTo")});await permanentlyDeleteAttachment(o.id,x.attachmentId,configuredAttachmentStorage().storage);revalidatePath(x.returnTo);}
