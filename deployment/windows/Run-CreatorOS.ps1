...
while ($true) {
    ...
    $process = Start-Process -FilePath $NodePath -ArgumentList @($next, "start", "-H", $HostName, "-p", [string]$Port) -WorkingDirectory $AppRoot -RedirectStandardOutput $stdout -RedirectStandardError $stderr -PassThru -Wait -WindowStyle Hidden
