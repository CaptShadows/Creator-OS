...
describe("Windows production deployment",()=>{
  ...
  it("stops an existing app before replacing locked dependencies and restores it on failure",()=>{const script=read("Install-CreatorOS.ps1"),stop=script.indexOf('Stop-ScheduledTask -TaskName "CreatorOS-App"'),install=script.indexOf('Invoke-CreatorOSCommand $npm @("ci"');expect(stop).toBeGreaterThan(0);expect(stop).toBeLessThan(install);expect(script).toContain('if ($existingAppWasRunning) { Start-ScheduledTask')});
});