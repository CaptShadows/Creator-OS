...
describe("Windows production deployment",()=>{
  ...
  it("uses a versioned Creator OS icon for the public desktop shortcut",()=>{const script=read("Install-CreatorOS.ps1");expect(script).toContain('CreatorOS-v2.ico');expect(script).toContain('FromBase64String');expect(script).toContain('Remove-Item -LiteralPath $shortcutPath');expect(script).toContain('IconLocation')});
  it("recreates the shortcut with a versioned icon during updates",()=>{const script=read("Update-CreatorOS.ps1");expect(script).toContain('CreatorOS-v2.ico');expect(script).toContain('Creator OS.lnk');expect(script).toContain('Remove-Item -LiteralPath $shortcutPath');expect(script).toContain('IconLocation')});
});
