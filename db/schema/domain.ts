...
export const platformAccounts = pgTable(
  ...
  (table) => [
    index("platform_accounts_owner_platform_idx").on(
      table.ownerUserId,
      table.platform,
    ),
    uniqueIndex("platform_accounts_external_unique").on(
      table.ownerUserId,
      table.platform,
...
export const contents = pgTable(
  ...
  (table) => [
    ...
    index("contents_owner_film_date_idx").on(
      table.ownerUserId,
      table.plannedFilmAt,
    ),
  ],
...
export const publications = pgTable(
  "publications",
  {
    ...
    platformCaptionOverride: text("platform_caption_override"),
    metadata: metadata(),
    ...timestamps,
  },
  (table) => [
    index("publications_owner_status_date_idx").on(
      table.ownerUserId,
      table.status,
      table.scheduledAt,
    ),
    index("publications_content_idx").on(table.contentId),
  ],
);
...
export const campaigns = pgTable(
  "campaigns",
  {
    ...
    briefReference: text("brief_reference"),
    notes: text("notes"),
    ...timestamps,
  },
  (table) => [
    index("campaigns_owner_status_due_idx").on(
      table.ownerUserId,
      table.status,
      table.dueAt,
    ),
...
export const brandDeals = pgTable(
  "brand_deals",
  {
    ...
    dealType: text("deal_type").default("paid").notNull(),
    status: text("status").default("lead").notNull(),
    priority: integer("priority").default(1).notNull(),
    startAt: timestamp("start_at", { withTimezone: true }),
    dueAt: timestamp("due_at", { withTimezone: true }),
    notes: text("notes"),
    fixedCompensationCents: integer("fixed_compensation_cents"),
    giftedValueCents: integer("gifted_value_cents"),
    ...
    paymentStatus: text("payment_status").default("not_due").notNull(),
    amountReceivedCents: integer("amount_received_cents").default(0).notNull(),
    receivedAt: timestamp("received_at", { withTimezone: true }),
    contractSigned: boolean("contract_signed").default(false).notNull(),
    contractSignedAt: timestamp("contract_signed_at", { withTimezone: true }),
    usageRights: text("usage_rights"),
    organicRights: boolean("organic_rights").default(false).notNull(),
    paidUsage: boolean("paid_usage").default(false).notNull(),
    exclusivityCategory: text("exclusivity_category"),
    exclusivityStartAt: timestamp("exclusivity_start_at", {
      withTimezone: true,
    }),
    exclusivityEndAt: timestamp("exclusivity_end_at", { withTimezone: true }),
  ...
  (table) => [
    index("brand_deals_owner_status_due_idx").on(
      table.ownerUserId,
      table.status,
      table.dueAt,
    ),
    index("brand_deals_owner_payment_idx").on(
      table.ownerUserId,
      table.paymentStatus,
      table.paymentDueAt,
    ),
...
export const brandDealDeliverables = pgTable(
  "brand_deal_deliverables",
  {
    ...
    ownerUserId: uuid("owner_user_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    brandDealId: uuid("brand_deal_id")
      .notNull()
      .references(() => brandDeals.id, { onDelete: "restrict" }),
    ...
    approvalStatus: text("approval_status").default("not_submitted").notNull(),
  ...
  (table) => [
    index("brand_deal_deliverables_owner_due_idx").on(
      table.ownerUserId,
      table.status,
      table.dueAt,
    ),
    index("brand_deal_deliverables_deal_idx").on(table.brandDealId),
  ],
);

export const brandDealCampaigns = pgTable(
  "brand_deal_campaigns",
  {
    ownerUserId: uuid("owner_user_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
  ...
  (table) => [
    primaryKey({ columns: [table.brandDealId, table.campaignId] }),
    index("brand_deal_campaigns_owner_idx").on(table.ownerUserId),
  ],
);
export const brandDealContents = pgTable(
  "brand_deal_contents",
  {
    ownerUserId: uuid("owner_user_id")
      .notNull()
  ...
  (table) => [
    primaryKey({ columns: [table.brandDealId, table.contentId] }),
    index("brand_deal_contents_owner_idx").on(table.ownerUserId),
  ],
);
...
export const deliverables = pgTable(
  ...
  (table) => [
    index("deliverables_owner_status_due_idx").on(
      table.ownerUserId,
      table.status,
      table.dueAt,
    ),
    index("deliverables_campaign_idx").on(table.campaignId),
  ],
);
...
export const products = pgTable(
  ...
  (table) => [
    index("products_owner_active_name_idx").on(
      table.ownerUserId,
      table.active,
      table.name,
    ),
...
export const productPlatformListings = pgTable(
  ...
  (table) => [
    index("product_listings_product_idx").on(table.productId),
    index("product_listings_owner_platform_idx").on(
      table.ownerUserId,
      table.platform,
    ),
  ],
...
export const samples = pgTable(
  "samples",
  {
    ...
    status: sampleStatus("status").default("requested").notNull(),
  ...
  (table) => [
    index("samples_owner_status_idx").on(table.ownerUserId, table.status),
...
export const compensations = pgTable(
  ...
  (table) => [
    index("compensations_owner_expected_idx").on(
      table.ownerUserId,
      table.expectedPaymentAt,
    ),
    index("compensations_campaign_idx").on(table.campaignId),
  ],
);
...
export const payments = pgTable(
  "payments",
  {
    ...
    campaignId: uuid("campaign_id")
      .notNull()
      .references(() => campaigns.id, { onDelete: "restrict" }),
    compensationId: uuid("compensation_id").references(() => compensations.id, {
      onDelete: "restrict",
    }),
  ...
  (table) => [
    index("payments_owner_status_due_idx").on(
      table.ownerUserId,
      table.status,
      table.dueAt,
    ),
    index("payments_compensation_idx").on(table.compensationId),
  ],
);
...
export const assetReferences = pgTable(
  ...
  (table) => [
    index("asset_references_owner_type_idx").on(table.ownerUserId, table.type),
    index("asset_references_content_idx").on(table.contentId),
  ],
);
...
export const attachments = pgTable(
  ...
  (table) => [
    ...
    index("attachments_owner_archived_idx").on(
      table.ownerUserId,
      table.archivedAt,
    ),
  ],
...
export const attachmentLinks = pgTable(
  "attachment_links",
  {
    ...
    brandDealId: uuid("brand_deal_id").references(() => brandDeals.id, {
      onDelete: "cascade",
    }),
    createdAt: timestamp("created_at", { withTimezone: true })
      .defaultNow()
...
export const integrationStates = pgTable(
  ...
  (table) => [
    uniqueIndex("integration_states_owner_integration_account_unique").on(
      table.ownerUserId,
      table.integration,
      table.platformAccountId,
    ),
    index("integration_states_owner_status_idx").on(
      table.ownerUserId,
      table.status,
    ),
  ],
...
export const contentCampaigns = pgTable(
  ...
  (table) => [
    primaryKey({ columns: [table.contentId, table.campaignId] }),
    index("content_campaigns_owner_idx").on(table.ownerUserId),
  ],
);
...
export const contentProducts = pgTable(
  ...
  (table) => [
    ...
    index("content_products_owner_idx").on(table.ownerUserId),
  ],
);

export const brandDealProducts = pgTable(
  "brand_deal_products",
  {
    ownerUserId: uuid("owner_user_id")
      .notNull()
  ...
  (table) => [
    primaryKey({ columns: [table.brandDealId, table.productId] }),
    index("brand_deal_products_owner_idx").on(table.ownerUserId),
  ],
);
export const brandDealSamples = pgTable(
  "brand_deal_samples",
  {
    ownerUserId: uuid("owner_user_id")
      .notNull()
  ...
  (table) => [
    primaryKey({ columns: [table.brandDealId, table.sampleId] }),
    index("brand_deal_samples_owner_idx").on(table.ownerUserId),
  ],
);
...
export const platformAccountsRelations = relations(
  ...
  ({ many }) => ({
    ...
    productListings: many(productPlatformListings),
  }),
);
export const contentsRelations = relations(contents, ({ many }) => ({
  publications: many(publications),
  deliverables: many(deliverables),
  assets: many(assetReferences),
  campaignLinks: many(contentCampaigns),
  productLinks: many(contentProducts),
}));
export const publicationsRelations = relations(publications, ({ one }) => ({
  content: one(contents, {
    fields: [publications.contentId],
    references: [contents.id],
  }),
  platformAccount: one(platformAccounts, {
    fields: [publications.platformAccountId],
...
export const deliverablesRelations = relations(deliverables, ({ one }) => ({
  ...
  content: one(contents, {
    fields: [deliverables.contentId],
    references: [contents.id],
  }),
}));
export const productsRelations = relations(products, ({ one, many }) => ({
  brand: one(brands, { fields: [products.brandId], references: [brands.id] }),
  listings: many(productPlatformListings),
...
export const samplesRelations = relations(samples, ({ one }) => ({
  ...
  content: one(contents, {
    fields: [samples.contentId],
    references: [contents.id],
  }),
  campaign: one(campaigns, {
    fields: [samples.campaignId],
    references: [campaigns.id],
  }),
}));
export const productListingsRelations = relations(
  ...
  ({ one }) => ({
    product: one(products, {
      fields: [productPlatformListings.productId],
      references: [products.id],
    }),
    platformAccount: one(platformAccounts, {
      fields: [productPlatformListings.platformAccountId],
      references: [platformAccounts.id],
    }),
  }),
);
export const compensationsRelations = relations(
  ...
  ({ one, many }) => ({
    campaign: one(campaigns, {
      fields: [compensations.campaignId],
      references: [campaigns.id],
    }),
    payments: many(payments),
  }),
...
export const paymentsRelations = relations(payments, ({ one }) => ({
  ...
  compensation: one(compensations, {
    fields: [payments.compensationId],
    references: [compensations.id],
  }),
}));
export const attachmentsRelations = relations(attachments, ({ many }) => ({
  links: many(attachmentLinks),
}));
export const attachmentLinksRelations = relations(
  attachmentLinks,
  ({ one }) => ({
